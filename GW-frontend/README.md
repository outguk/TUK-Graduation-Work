...
- 회원가입 시 이메일을 받을 수 있도록 회원가입 페이지에 이메일 입력란 추가
- 회원가입 시 가입일자를 저장, UserProfilePage에서 가입일자를 볼수 있도록 변경
...
- 비언어 평가 페이지 추가. 대시보드에서 상세 보기를 누르면 해당 분석에 대한 평가 결과가 나오도록 수정.
- 실제 영상을 가져오는 부분에 오류가 많이 떠서 일단은 타임라인 바를 누르면 해당 구간에 대한 썸네일이 나오도록 설정

# 4/16 업데이트

## 변경 사항
- UploadPage 디자인 제작. 업로드 요소는 건드린게 아예 없음, 백엔드 부분도 db 설정 파일 정보 삭제한 것이 변경된 것으로 뜨는 것
- 이미지 부분은 직접 대시보드와 상세 페이지를 캡쳐해서 넣을 생각이며 주의사항 부분에 들어갈 이미지는 AIhub의 얼굴 모자이크된 이미지 넣으면 될듯
  
# 5/12 업데이트

## 변경 사항
- 대시보드 말하기 속도 부분 게이지 바 색깔 을 구간별로 변경해 직관성 강화
- 대시보드 요소들 한글로 수정
- 비언어 세부 페이지 구간별로 영상 넘어가도록 로직 수정

# 6/21 업데이트

## 변경 사항
- 대시보드 구조 변경 및 디자인 변경
- 언어, 비언어 분석 페이지 피드백 요소 강조

## 추가할 사항
- 대시보드 비언어 부분 이미지 변경 필요
- 메인 페이지 디자인 추가

- 메인 페이지 "안녕하세요!(사용자)" 여기에 사용자 아이디 넣기
- 사이드 바 대본과 영상 분석 별도 분리
- 사이드 바 총 영상 시간 반영 안되는 현상
