...
//         },
//       };
//       console.log('Upload request config:', config);
//       console.log('Uploading file:', file.name);
//       const response = await axios.post('http://localhost:5000/upload-video', formData, config);
...
//       if (e.response) console.log('Error response:', e.response.data);
//     } finally {
//       setLoading(false);
//     }
//   };
...
const UploadPage: React.FC = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      console.log('Upload request config:', config);
      console.log('Uploading file:', file.name);
      const response = await axios.post('http://localhost:8080/spring/api/upload', formData, config);
      console.log("📦 분석 응답:", response.data);
      setResult(response.data);
    } catch (e: any) {
      const errorMessage = e.response?.data?.message || "업로드 또는 분석 중 오류가 발생했습니다.";
  ...
  return (
    <Container maxWidth="lg">
      <Box component="main" mt={6} mb={5} p={4} borderRadius={3} boxShadow={4} bgcolor="background.paper">
        ...
        <Stack spacing={3} alignItems="center">
          <Box width="100%">
            <Typography variant="subtitle1" gutterBottom>MP4 파일 선택</Typography>
            <input
              id="video-upload"
              type="file"
              accept="video/mp4"
              ...
              style={{ display: 'block', marginBottom: 8 }}
            />
            <Typography variant="body2" color="text.secondary">최대 500MB까지 업로드 가능합니다.</Typography>
          </Box>
          <Button
            variant="contained"
            color="primary"
        ...
        {error && (
          <Typography color="error" mt={3} align="center">{error}</Typography>
        )}
      </Box>

      {result && (
        <Box mt={6}>
          <Typography variant="h5" gutterBottom align="center">분석 결과 (최근 업로드 파일)</Typography>
          <Box mt={3} sx={{ display: 'flex', gap: 3, overflowX: 'auto', pb: 2 }}>
            <Card sx={{ minWidth: 320, borderRadius: 3, boxShadow: 3 }}>
              <CardContent>
                <Typography variant="subtitle1" gutterBottom>🗣 말하기 속도 분석</Typography>
                      ...
                      <TableRow key={key}>
                        ...
                        <TableCell>
                          {!item.is_normal ? (
                            <ul style={{ paddingLeft: 16 }}>
                              {item.top_classes.map((cls, i) => (
                                <li key={i}>{cls.class} ({cls.probability}%)</li>
                              ))}
                            </ul>
                          ) : '-'}
                        </TableCell>
          ...
          {result?.script_analysis && (
            <Box mt={5}>
              <Typography variant="h6" gutterBottom>📝 대본 분석 결과</Typography>
              <Paper elevation={2} sx={{ borderRadius: 2, p: 2, backgroundColor: "#f8f9fa" }}>
                <Typography variant="body1" gutterBottom>{result.script_analysis.length_feedback}</Typography>
                <Typography variant="body2" mt={1}>
                  🔢 전체 글자 수: {result.script_analysis.length}자<br />
                  📏 권장 범위: {result.script_analysis.min_length} ~ {result.script_analysis.max_length}자
                </Typography>
