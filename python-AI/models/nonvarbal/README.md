...
# 비언어 행동이 감지되지 않은 영상의 경우 오류 발생

: 비언어 분석 결과가 없는 경우의 처리 방식

-> 기존 : step4에서 test 실행시 오류 발생 및 DB 저장 오류

-> 해결 : 움직임 변화량이 기준(threshold)을 초과하는 프레임이 없을 경우(비언어 행동이 없는 경우) 분석 대상이 될 만한 비언어적 행동이 없다고 판단하여 ST-GCN++ 테스트를 생략함.

- 프론트엔드는 `results.length === 0` 여부를 기준으로 처리하면 됨

```json
"nonverbal_analysis": {
  "status": "ok",
  "message": "유효한 비언어적 행동 없음",
  "results": []
}
```
