...
class RecognizerGCN(BaseRecognizer):
    ...
    def extract_feat(self,
        ...
        # print(f"[INFO] RecognizerGCN")
        # print(f"[INFO] input shzpe before reshape : {inputs.shape}")
        bs, nc = inputs.shape[:2]
        inputs = inputs.reshape((bs * nc, ) + inputs.shape[2:])

        # print(f"[INFO] input shzpe after reshape : {inputs.shape}")
        # origin shape (16, 2, 100, 17, 3)
